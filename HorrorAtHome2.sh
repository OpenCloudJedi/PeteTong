# This is for server2 in the at home Tails guide
#!/bin/bash
########################################################
#  Global Variables ####################################
...
TARFILE="/root/DarkLibrary.tar.bz2"
...
vdo list | grep -q vdoNightmare
  RESULT=$?
      if [ "${RESULT}" -ne 0 ]; then
                print_FAIL
                echo -e "\033[1;31m - VDO volume vdoNightmare unavailable. \033[0;39m"
              return 1
      fi
vdo status --name=vdoNightmare | grep -q "Logical size: 50G"
  RESULT=$?
      if [ "${RESULT}" -ne 0 ]; then
                print_FAIL
                echo -e "\033[1;31m - VDO volume vdoNightmare doesn't have a logical size of 50G. \033[0;39m"
              return 1
      fi
  print_PASS
  return 0
...
stratis pool list | grep -q ThinClient
RESULT=$?
      if [ "${RESULT}" -ne 0 ]; then
                print_FAIL
                echo -e "\033[1;31m - Stratis pool ThinClient does not exist. \033[0;39m"
              return 1
      fi
stratis filesystem | grep -q Starved
RESULT=$?
      if [ "${RESULT}" -ne 0 ]; then
                print_FAIL
                echo -e "\033[1;31m - Stratis filesystem called Starved was not found. \033[0;39m"
              return 1
      fi
  print_PASS
...
function grade_swap() {
  ...
  read PART TYPE SIZE USED PRIO <<< $(swapon -s | grep -v /dev/vdb1 2>/dev/null | tail -n1 2>/dev/null) 2>/dev/null
